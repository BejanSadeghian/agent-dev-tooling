...
test('maps a file inside a skill back to the skill', () => {
  assert.equal(skillForPath(config, '.claude/skills/skill-builder/references/interview.md'), 'skill-builder');
  assert.equal(skillForPath(config, 'examples/skills/hello-linter/SKILL.md'), 'hello-linter');
});

test('returns null for files outside any skills root', () => {
  assert.equal(skillForPath(config, 'scripts/validate-skill.mjs'), null);
  assert.equal(skillForPath(config, 'README.md'), null);
  assert.equal(skillForPath(config, '.claude/skills/.hidden/SKILL.md'), null);
});
...
test('every configured skills root is discovered', () => {
  ...
  assert.ok(names.includes('hello-linter'));
});
