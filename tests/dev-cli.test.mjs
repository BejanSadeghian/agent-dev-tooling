...
const dev = (dir, args, env = {}) =>
  spawnSync('node', ['scripts/dev.mjs', ...args], {
    cwd: dir,
    encoding: 'utf8',
...
test('save refuses when the checks fail, and nothing is committed', (t) => {
  ...
  // Break a rule the skill's own regression suite asserts: `npm run save` runs the
  // checks first, so this must never reach a commit.
...
test('save commits with a conventional message once the checks pass', (t) => {
  ...
  fs.appendFileSync(path.join(dir, 'docs/GUIDE-FOR-HUMANS.md'), '\nA clarifying sentence.\n');
