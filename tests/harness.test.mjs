...
test('the Python test harness passes its own tests', { skip: !hasPython() && 'python3 not installed' }, () => {
  const result = spawnSync('python3', ['-m', 'unittest', 'discover', '-s', 'harness/tests', '-t', 'harness/tests'], {
    cwd: REPO_ROOT,
    encoding: 'utf8',
    env: { ...process.env, PYTHONPATH: path.join(REPO_ROOT, 'harness'), PYTHONDONTWRITEBYTECODE: '1' },
  });
