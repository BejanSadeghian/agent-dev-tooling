...
- **Every use case is a pair.** The **doer** is as low-level and procedural as the task allows:
  ...
  `references/schema.md` — easy to verify exactly. The **observer** reads that artifact,
...
- create a new use case (a doer/observer pair) from raw material — notes, a transcript, a runbook, an existing prompt, "the way I do X";
...
npm run subagent -- <use-case> "<the real task>" --role observer # observer, after the doer
...
| "It mixed opinion into the facts" | a sharper Facts rule in the observer **and** a structural case |
...
spec in `.framework/framework-testing.md`). `npm run scenario -- <use-case>` runs it three times in fresh
