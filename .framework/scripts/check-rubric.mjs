...
export function checkRubric(config, skill, root = REPO_ROOT) {
  ...
  if (rubric.requireArtifactCoverage || rubric.requirePythonCoverage) {
    for (const gap of coverageGaps(config, skill, cases)) {
  ...
  if (rubric.requirePerfMeasurements && modules.length && hasPythonCode(config, skill.dir)) {
    ...
    if (!perf || perf.corrupt) {
      ...
      if ((perf.measurements ?? []).length === 0) {
        err(`R10 nothing was measured — the performance test must call self.measure(...)`, perfRel);
      }
      for (const m of perf.measurements ?? []) {
        const budget = config.python.maxExponentDefault ?? config.perf.maxExponentDefault;
