...
const SKIP_DIRS = new Set(['node_modules', '.git', '__pycache__', '.pytest_cache', 'tmp', 'outputs', 'runs']);
...
export const RULES = [
  ...
  },
  {
    ...
    applies: (rel) => /\.(mjs|py)$/.test(rel) && rel !== '.framework/scripts/lint.mjs' && !rel.startsWith('.framework/tests/'),
    check: ({ text }) => {
  ...
  },
  {
    ...
    applies: (rel) => /^\.framework\/scripts\/[^/]+\.mjs$/.test(rel),
    check: ({ text }) =>
  ...
  },
  {
    ...
    applies: (rel) =>
      rel.startsWith('.framework/scripts/lib/') &&
      rel.endsWith('.mjs') &&
      !['.framework/scripts/lib/report.mjs', '.framework/scripts/lib/prompt.mjs'].includes(rel),
    check: ({ text }) =>
      text
        .split('\n')
  ...
  },
  {
    ...
    applies: (rel) => rel.startsWith('.framework/harness/skillharness/') && rel.endsWith('.py'),
    check: ({ text }) =>
      [
        ['random.', /\brandom\.\w+\(/],
...
function optionalEslint(root) {
  ...
  const result = spawnSync(binary, ['--config', '.framework/eslint.config.mjs', '.'], { cwd: root, encoding: 'utf8' });
