...
test('no-unfinished-markers fires on code but exempts tests and the rule file itself', () => {
  assert.match(run('no-unfinished-markers', '.framework/scripts/thing.mjs', '// TODO finish\n')[0], /TODO/);
  assert.deepEqual(run('no-unfinished-markers', '.framework/tests/a.test.mjs', '// TODO as a fixture\n'), []);
  assert.deepEqual(run('no-unfinished-markers', '.framework/scripts/lint.mjs', '// TODO\n'), []);
});

test('cli-has-main-guard fires only on a script that defines main() without the guard', () => {
  const guarded = 'function main() {}\nif (import.meta.url === `file://${process.argv[1]}`) process.exit(main());\n';
  assert.deepEqual(run('cli-has-main-guard', '.framework/scripts/a.mjs', guarded), []);
  assert.match(run('cli-has-main-guard', '.framework/scripts/a.mjs', 'function main() {}\nmain();\n')[0], /no import\.meta\.url/);
  assert.deepEqual(run('cli-has-main-guard', '.framework/scripts/a.mjs', 'export const x = 1;\n'), []);
  assert.deepEqual(run('cli-has-main-guard', '.framework/scripts/lib/a.mjs', 'function main() {}\n'), []);
});

test('libraries-do-not-print fires in lib/ but not in the CLIs or the reporter', () => {
  assert.match(run('libraries-do-not-print', '.framework/scripts/lib/a.mjs', 'console.log("x");\n')[0], /console output/);
  assert.deepEqual(run('libraries-do-not-print', '.framework/scripts/lib/report.mjs', 'console.log("x");\n'), []);
  assert.deepEqual(run('libraries-do-not-print', '.framework/scripts/cli.mjs', 'console.log("x");\n'), []);
  assert.deepEqual(run('libraries-do-not-print', '.framework/scripts/lib/a.mjs', 'const s = fake.console.log;\n'), []);
});

test('harness-is-deterministic fires on clock and randomness in the harness', () => {
  assert.match(run('harness-is-deterministic', '.framework/harness/skillharness/perf.py', 'import random\nrandom.random()\n')[0], /not reproducible/);
  assert.match(run('harness-is-deterministic', '.framework/harness/skillharness/perf.py', 'time.time()\n')[0], /time\.time/);
  assert.deepEqual(run('harness-is-deterministic', '.framework/harness/skillharness/perf.py', 'time.perf_counter()\n'), []);
