...
if [ ! -f .framework/setup/reviewers.json ]; then
  warn "no reviewers configured yet — run: bash .framework/setup/configure-gh.sh"
fi

echo
echo "Running the checks once to prove it works"
node .framework/scripts/verify-all.mjs
...
echo "  npm run publish -- <use-case>   confirm checks, save, and deliver"
