...
|---|---|---|
| The skill's `skill.json` | yes | Declares the artifacts and entrypoints that must be covered. |
| The code under test | yes | Read `python/` before deciding what to assert. |
...
It reads `skill.json`, maps every artifact and entrypoint against the tests that declare they cover
it, and writes the file that closes each gap. What it writes are working scaffolds with real
...
```bash
node .claude/skills/test-generator/scripts/datagen.mjs --spec spec.json --seed 42 --rows 200 --format ndjson --out fixtures/users.ndjson
node .claude/skills/test-generator/scripts/datagen.mjs --spec spec.json --seed 42 --rows 0 --edge-cases --format csv
```
