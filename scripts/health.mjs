...
import { checkSequences, duplicateArtifactIds } from './lib/sequences.mjs';
...
export function inspect(config, root = REPO_ROOT, now = Date.now()) {
  ...
  for (const skill of skills) {
    ...
    const gaps = manifest ? coverageGaps(config, skill.dir, manifest, cases) : [];
  ...
  for (const dup of duplicateArtifactIds(config, root)) {
    note('error', dup.skills.join(' + '), `both claim to produce the artifact "${dup.id}"`, 'rename one, or make one consume the other');
  }
  for (const finding of checkSequences(config, root)) {
    note(finding.level, finding.where, finding.message, 'fix the chain in sequences/');
  }
...
function renderMarkdown({ rows, issues }) {
  const lines = ['### Skill library health', '', '| skill | kind | artifacts | cases | python | tested | green | gaps | worst scaling |', '|---|---|---:|---:|---|---|---|---:|---|'];
  for (const r of rows) {
    lines.push(
      `| ${r.skill} | ${r.kind} | ${r.artifacts} | ${r.cases} | ${r.python} | ${
...
function main(argv) {
  ...
  if (argv.includes('--markdown')) {
    ...
    console.log(dim(`${'skill'.padEnd(width)}  artifacts  cases  python  tested        green  gaps`));
    for (const r of result.rows) {
      const tested = r.testedDaysAgo === null ? 'never' : r.fresh ? `${r.testedDaysAgo}d ago` : 'STALE';
      const mark = r.ok && r.green ? green('ok ') : red('!! ');
      console.log(
        `${mark}${r.skill.padEnd(width)}  ${String(r.artifacts).padStart(9)}  ${String(r.cases).padStart(5)}  ${r.python.padEnd(6)}  ${tested.padEnd(12)}  ${
