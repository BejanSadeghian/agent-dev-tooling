...
const hooksWired = () => git(['config', 'core.hooksPath'], { allowFail: true }) === 'hooks';

function runChecks({ quiet = false } = {}) {
  const result = spawnSync('node', ['scripts/verify-all.mjs'], {
    cwd: REPO_ROOT,
    encoding: 'utf8',
...
async function save(argv, prompter) {
  ...
  if (!hooksWired()) {
    ...
    execFileSync('bash', ['scripts/setup-hooks.sh'], { cwd: REPO_ROOT, stdio: 'inherit' });
  }
...
function reviewers() {
  const file = path.join(REPO_ROOT, 'setup/reviewers.json');
...
async function ship(argv, prompter) {
  ...
  if (!has('gh')) {
    ...
    say(dim('Or install it and run: bash setup/configure-gh.sh'));
    return 0;
  }
  ...
  if (result.status !== 0) {
    ...
    say(dim('If it mentions authentication, run: bash setup/configure-gh.sh'));
    return 1;
  }
  say(green(`\nReview request opened: ${result.stdout.trim()}`));
  if (people.length) say(dim(`Reviewers asked: ${people.join(', ')}`));
  else say(yellow('No reviewers are configured yet — run: bash setup/configure-gh.sh'));
  return 0;
}
...
function doctor() {
  ...
  record('GitHub command line (optional)', has('gh'), 'install gh, then run: bash setup/configure-gh.sh');

  const wired = hooksWired();
  if (!wired) {
    execFileSync('bash', ['scripts/setup-hooks.sh'], { cwd: REPO_ROOT, stdio: 'ignore' });
  }
  record('safety checks run before each save', hooksWired(), 'run: bash scripts/setup-hooks.sh');
  record('the hook script can run', fs.existsSync(path.join(REPO_ROOT, 'hooks/pre-commit')), 'the file hooks/pre-commit is missing');
  record('reviewers are configured', reviewers().length > 0, 'run: bash setup/configure-gh.sh');
  record(
    'the test harness loads',
    spawnSync('python3', ['-c', 'import skillharness'], {
      cwd: REPO_ROOT,
      env: { ...process.env, PYTHONPATH: path.join(REPO_ROOT, 'harness') },
      stdio: 'ignore',
    }).status === 0,
    'run from the repo folder, or reinstall with: bash setup/install.sh',
  );
